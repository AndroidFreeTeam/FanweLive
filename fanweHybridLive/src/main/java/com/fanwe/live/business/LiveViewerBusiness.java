...
public class LiveViewerBusiness extends LiveBusiness
{
    ...
    protected void onRequestRoomInfoSuccess(App_get_videoActModel actModel)
    {
        ...
        if(actModel.getTop_list_switch() == 1){
            mBusinessCallback.onBsViewerShowTopList(true);
        }else{
            mBusinessCallback.onBsViewerShowTopList(false);
        }
    }
     ...
     * @param callback
     */
    public void requestPrevRoom(int roomId, AppRequestCallback<Video_check_statusActModel> callback)
    {
        CommonInterface.requestPrevRoom(roomId, null, new AppRequestCallbackWrapper<Video_check_statusActModel>(callback)
        {
            @Override
            public String getCancelTag()
            {
     ...
     * @param callback
     */
    public void requestNextRoom(int roomId, AppRequestCallback<Video_check_statusActModel> callback)
    {
        CommonInterface.requestNextRoom(roomId, null, new AppRequestCallbackWrapper<Video_check_statusActModel>(callback)
        {
            @Override
            public String getCancelTag()
            {
    ...
    public interface LiveViewerBusinessCallback extends LiveBusinessCallback
    {
         ...
         * @param show
         * */
        void onBsViewerShowTopList(boolean show);
    }
