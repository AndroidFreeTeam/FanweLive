...
public class LiveMainHomeView extends BaseAppView
{
    ...
    private void clickService(){
        ...
        if(null != initActModel){
            String service_jump_url = initActModel.getService_jump_url();
           if(!TextUtils.isEmpty(service_jump_url)){
               Intent intent = new Intent(getActivity(), LiveWebViewActivity.class);
               intent.putExtra(LiveWebViewActivity.EXTRA_URL, service_jump_url);
               getActivity().startActivity(intent);
           }
        }
    }
